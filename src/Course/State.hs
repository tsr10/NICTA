...
instance Apply (State s) where
  (<*>) ::
    ...
    -> State s a
    -> State s b 
  (<*>) (State j) (State k) = State (\s -> let (a, t) = j s 
                                               (g, z) = k t 
                                        in (a g, z))
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = eval (findM (\x -> State (S.member x &&& S.insert x)) xs) S.empty
...
isHappy ::
  Integer
  -> Bool
isHappy xs = if contains 1 (eval (findM (\x -> State (S.member x &&& S.insert x)) (produce squareDigits xs)) (S.singleton 1)) then True else False

squareDigits ::
  Integer
  -> Integer
squareDigits xs = toInteger (foldRight (\x y -> digitToInt(x) * digitToInt(x) + y) 0 (show' xs))
